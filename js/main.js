window.onload = function() {
    ...
    var swipe = ['down','right','rotate'];
    var color = ['rgb(231, 71, 47)','rgb(82, 160, 164)','rgb(192, 177, 22)','rgb(93, 155, 152)','rgb(178, 66, 55)','rgb(217, 204, 100)','rgb(60, 86, 111)','rgb(231, 57, 20)','rgb(230, 230, 228)','rgb(255, 255, 255)','rgb(236, 114, 125)','rgb(5, 61, 118)','rgb(246, 215, 73)','rgb(250, 222, 0)','rgb(231, 71, 47)']
    function doit(){
        var dparent = document.getElementById("container");
        for (var i = 0; i < img_data.data.length-10; i++) {
            ...
            var swipe_way = Math.floor(Math.random()*swipe.length);
            boximg.className = "box_img swipe-" + swipe[swipe_way];//为节点添加类名
            var curtain = document.createElement("div");//创建节点
            curtain.className = "curtain";//为节点添加类名
            var bg_index = Math.floor(Math.random()*color.length);
            curtain.style.cssText = "background:" + color[bg_index] + ';';
            dcontent.appendChild(boximg);//添加根元素
            boximg.appendChild(curtain);//添加根元素
            var img = document.createElement("img");//创建节点
            var index = Math.floor(Math.random()*(img_data.data.length-1));
            img.src = "images/" + img_data.data[index].src;//图片加载路径
